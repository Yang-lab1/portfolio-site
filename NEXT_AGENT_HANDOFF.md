...
- 2026-07-01 追加优化：用户反馈快速下滑时图片显现仍慢于滚动速度，因此 `expansionCards` 的 9 张圆盘轻量图已提前到首页 warmup 队列前段，并在圆盘组件自身加了约 650ms 的低优先级 blob 预取保险；仍遵守省流量 / 2G 跳过策略。
...
- 首轮加载问题已本地修复：底部圆盘双层图片不再 eager 抢首屏网络，首页圆盘使用 640 WebP 展示版，并在圆盘组件自身约 650ms 后提前低优先级预取 9 张圆盘 blob。
