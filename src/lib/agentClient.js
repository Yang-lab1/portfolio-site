const ALLOWED_MODES = new Set(['navigate', 'answer', 'answer_with_navigation', 'clarify', 'not_found']);
...
function scoreSemanticCandidate(candidate, query) {
  ...
  if (titleZh.includes(compactQuery) || titleEn.includes(compactQuery) || projectId.includes(compactQuery)) score += 40;
  ...
  (candidate.aliases || []).forEach((alias) => {
    ...
    if (compactQuery.includes(compactAlias) || compactAlias.includes(compactQuery)) {
      score += compactAlias.length > 2 ? 22 : 8;
    }
  });
...
function detectIntent(query, ranked) {
  ...
  const hasStrongProject = topScore >= 48;
...
export function resolveAgentFallbackDecision({ query, lang = 'zh', profile, candidates = [] }) {
  ...
  if (intent.asksProfile && (!intent.hasStrongProject || (top && second && top.semanticScore - second.semanticScore < 8))) {
    return {
      mode: 'answer',
      answer: buildProfileAnswer(profile, lang, query),
      projectId: null,
      relatedProjectIds: [],
      reasoningSummary: ['profile-intent', 'local-fallback'],
      confidence: 0.72,
    };
  }
  ...
  if (!top || !intent.hasStrongProject) {
    return {
      ...
      projectId: null,
      relatedProjectIds: dedupeProjectIds(ranked, 3),
