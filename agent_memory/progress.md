...
- 验证已通过：`npm run build`；桌面 1440x1100 本地生产版打开 1 秒圆盘加载数 `0`，接近圆盘区后切换为 `eager` + `high` 并在约 2.5 秒内 8/8 加载完成，横向溢出 `0`，控制台错误 `0`。
- 移动端 390x844 验证通过：打开初期圆盘图加载数 `0`，接近圆盘区后 8/8 WebP 圆盘图加载完成，横向溢出 `0`，控制台错误 `0`。
