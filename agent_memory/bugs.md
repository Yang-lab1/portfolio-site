...
## 已修复问题
- 2026-07-01：首屏会提前加载页面底部 Product Language 圆盘 8 张 1254 方图，合计约 `10.3MB`。已将首页圆盘切换到 8 张 640 WebP 展示版，合计约 198KB；圆盘图片节点初始不挂载，页面空闲后通过同源 `fetch` 预取为 blob，接近圆盘区后再挂载图片节点。打开初期圆盘图片节点数为 `0`，接近圆盘区后 8 张圆盘图全部加载完成。
