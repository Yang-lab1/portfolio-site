...
- User approved the v4 slow-drag proof for release.
- Runtime commit `ce87990` was pushed to GitHub.
- Production deployment `dpl_9DtW7Tk77UjdB6BUaP1if4kVR3HE` completed on Vercel and is aliased to `https://portfolio-site-three-rose.vercel.app/`.
...
- User approved push/deploy. Production deployment `dpl_ESPAPb6wNsNrW3SFTHBFjxFVD86W` is aliased to `https://portfolio-site-three-rose.vercel.app/`; public QA passed with CSS asset `index-BeFfQURw.css`, screenshot `tmp/about-metrics-bottom-clean-production/about-metrics-bottom-clean-production.png`, horizontal overflow `0`, and browser error count `0`.
...
# 2026-07-13 Portable Detail Header Findings
- The right-side whitespace was caused by both desktop `.detail-hero` grid tracks having fixed maximum widths while the grid container itself stayed full width.
- Keeping the metadata track fixed and allowing the title track to consume remaining width restores equal outer margins without changing the internal metadata density.
- Default media labels are generated by `getDetailMediaLabel`; project-level opt-out is safer than hiding captions globally.


## WasteFlow Phase 2 findings (2026-09-12)
- Reference boards are 7680x3291 (21:9), bg orange #F6A528, panel black #232323, card fill #E59435, cream text #F3EAD2.
- cqw pitfall: container-type must sit on .wf-casestudy (ancestor), not .wf-board; otherwise units resolve against a narrower ancestor container and everything renders ~6% small.
